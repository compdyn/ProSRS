...
def run(prob, n_iter, n_proc, outdir, init_iter=None, seed=1, save_samp=False, 
    ...
    Input:
        ...
        n_proc: integer, number of processes (cores) for parallel optimization.
    ...
    opt_iter = n_iter-init_iter # number of iterations for DOE
    n_proc = int(n_proc)
    seed = int(seed)
    assert(seed >= 0)
    ...
    for k in range(start_iter,opt_iter):
        ...
        if not full_restart:
            ...
            rbf_mod,opt_sm,cv_err,_ = RBF_reg(X_samp,Y_samp,n_proc,sm_range,
            ...
            if n_worker > 1:
                wgt_pat_arr = np.linspace(wgt_pat_bd[0],wgt_pat_bd[1],n_worker)
            else:
        ...
        assert(len(prop_pt_arr) == len(Y_prop_pt) == n_worker)
        ...
        # update node
        n_X_all = len(X_all)
        act_node['ix'] = np.append(act_node['ix'], np.arange(n_X_all,n_X_all+n_worker,dtype=int))
        # update samples
        ...
        if not full_restart:
            ...
            if n_worker > 1 or (n_worker == 1 and run_wgt_SRS == wgt_pat_bd[0]):
        ...
        np.savez(temp_result_npz_file,
                 ...
                 init_iter=init_iter,opt_iter=k+1,n_proc=n_proc,seed=seed,outdir=outdir,save_samp=save_samp,verbose=verbose,
                 ...
                 X_all=X_all,Y_all=Y_all,tree=tree,n_full_restart=n_full_restart,n_worker=n_worker,
