...
def func_eval(f,x_list,seed_list,out_list,pool):
    '''
    ...
    '''
    # sanity check
    assert(len(x_list)==len(seed_list)==len(out_list))
    
    x_seed_f_out_list = [(x,seed,f,out) for x,seed,out in zip(x_list,seed_list,out_list)]
    if pool is None:
        y_list = [eval_wrapper(x_seed_f_out) for x_seed_f_out in x_seed_f_out_list]      
    else:
    ...
    if comm is None or comm.rank == 0:
        ...
        if resume_iter is None:
            ...
            for k in range(init_iter):
                ...
                t1, t1_c = default_timer(), clock()
                # evaluate function
                Y = func_eval(obj_func,X,seed_arr,out_list,pool_eval)
                t2, t2_c = default_timer(), clock()
        ...
        for k in range(start_iter,opt_iter):
            ...
            out_list = [os.path.join(outdir, RESULT_SAMP_FILE_TEMP % (prob_name,k+init_iter+1,n+1)) \
                        for n in range(n_proc)]
            t1, t1_c = default_timer(), clock()
            Y_prop_pt = func_eval(obj_func,prop_pt_arr,seed_arr,out_list,pool_eval)
            t2, t2_c = default_timer(), clock()
